...
| 10 | プロフィール編集画面 | 自分のアイコン・表示名・自己紹介を編集 | 必要 |
...
flowchart LR
  ...
  MyPage -->|プロフィールを編集| ProfileEdit[プロフィール編集画面]
...
**主な操作**: 投稿作成（テキスト＋任意で画像1枚） / いいね・いいね解除 / 投稿編集・削除（自分の投稿のみ） / 投稿クリックで投稿詳細画面へ
...
**主な操作**: フォロー／フォロー解除（他ユーザー画面のみ） / フォロー数・フォロワー数クリックで一覧画面へ / プロフィール編集画面へ遷移（自分の画面のみ）
...
**主要表示項目**: アイコン画像（変更ボタン付き） / 表示名入力欄 / 自己紹介入力欄 / 保存ボタン
...
**主な操作**: アイコン画像の選択・プレビュー / 表示名・自己紹介の編集 / 保存（成功時マイページ画面へ戻る）
